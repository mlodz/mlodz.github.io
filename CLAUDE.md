...
4. Add a card to the root `index.html` `.topics` grid linking to `./<slug>/index.html` (use the explicit `index.html`, not a trailing slash — the site needs to work when opened via `file://` for local previewing).
